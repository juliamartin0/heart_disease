...
The performance of the backward selection model improved slightly to 0.73, but since the AUC only increased by a small margin, it wasn't worth adding all the extra complexity (as we can see, after six variables, the AUC score barely increases). During model optimization, I also experimented with various variable transformations to better capture relationships between the predictors and the outcome. While some transformations slightly improved model performance, the overall gains were minimal, so I opted for a simpler model with fewer variables for clarity and effectiveness. In this case, a simpler model with fewer variables still provided comparable performance (see image below), which aligns with the **principle of Occam’s Razor** — simpler models are often preferred when the performance difference is minimal.
